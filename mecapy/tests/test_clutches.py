...
class TestDiscClutchConstruction:
    ...
    def test_lining_supplies_mu(self):
        clutch = DiscClutch(outer_diameter=300, inner_diameter=225, lining="molded")
        assert clutch.mu == pytest.approx(0.35)

    def test_explicit_mu_wins_over_lining(self):
        clutch = DiscClutch(outer_diameter=300, inner_diameter=225, mu=0.2, lining="molded")
        assert clutch.mu == pytest.approx(0.2)

    def test_invalid_inputs(self):
        with pytest.raises(ValueError):
            DiscClutch(outer_diameter=0, inner_diameter=225)
        with pytest.raises(ValueError):
            DiscClutch(outer_diameter=300, inner_diameter=0)
        with pytest.raises(ValueError):
            DiscClutch(outer_diameter=225, inner_diameter=300)  # inverted
        with pytest.raises(ValueError):
            DiscClutch(outer_diameter=300, inner_diameter=225, n_faces=0)
        with pytest.raises(ValueError):
            DiscClutch(outer_diameter=300, inner_diameter=225, mu=2.0)
        with pytest.raises(ValueError):
            DiscClutch(outer_diameter=300, inner_diameter=225, lining="unobtainium")


class TestFrictionRadiusConvention:
    """D4/6.3: the friction family is radius-canonical, diameter accepted."""

    def test_radius_and_diameter_are_equivalent(self):
        """A radius pair and the matching diameter pair build the same clutch."""
        by_radius = DiscClutch(outer_radius=150, inner_radius=112.5, mu=0.25)
        by_diam = DiscClutch(outer_diameter=300, inner_diameter=225, mu=0.25)
        assert by_radius.outer_diameter == pytest.approx(by_diam.outer_diameter)
        assert by_radius.torque_uniform_wear(1000) == pytest.approx(
            by_diam.torque_uniform_wear(1000))

    def test_both_geometry_pairs_rejected(self):
        """Giving radii and diameters together raises."""
        with pytest.raises(ValueError):
            DiscClutch(outer_radius=150, inner_diameter=225)

    def test_radius_is_settable_and_revalidates(self):
        """The radius setter recomputes geometry and rejects bad values."""
        clutch = DiscClutch(outer_radius=150, inner_radius=100, mu=0.25)
        clutch.outer_radius = 200
        assert clutch.outer_diameter == pytest.approx(400)
        with pytest.raises(ValueError):
            clutch.inner_radius = 250  # exceeds outer radius
...
class TestDiscClutchOptimum:
    ...
    def test_is_optimal_ratio(self):
        optimal = DiscClutch(outer_diameter=300,
                             inner_diameter=300 / math.sqrt(3), mu=0.25)
    ...
    def test_optimum_maximizes_torque_capacity(self):
        ...
        t_star = DiscClutch(outer_diameter=300,
                            inner_diameter=d_star).torque_capacity_uniform_wear(1.0)
        for d in (0.9 * d_star, 1.1 * d_star):
            assert DiscClutch(outer_diameter=300, inner_diameter=d
                              ).torque_capacity_uniform_wear(1.0) < t_star
...
class TestConeClutch:
    def test_flat_cone_matches_disc(self):
        ...
        flat = AxialFrictionInterface(outer_diameter=300, inner_diameter=225,
                                      mu=0.25, cone_angle=90.0)
        disc = DiscClutch(outer_diameter=300, inner_diameter=225, mu=0.25)
    ...
    def test_wedging_amplifies_torque(self):
        cone = ConeClutch(outer_diameter=300, inner_diameter=225, cone_angle=12, mu=0.25)
        disc = DiscClutch(outer_diameter=300, inner_diameter=225, mu=0.25)
    ...
    def test_face_width(self):
        cone = ConeClutch(outer_diameter=300, inner_diameter=225, cone_angle=12)
    ...
    def test_self_holding(self):
        ...
        holding = ConeClutch(outer_diameter=300, inner_diameter=225,
                             cone_angle=8, mu=0.3)
        free = ConeClutch(outer_diameter=300, inner_diameter=225,
                          cone_angle=30, mu=0.3)
        assert holding.is_self_holding
        assert not free.is_self_holding

    def test_invalid_cone_angle(self):
        with pytest.raises(ValueError):
            ConeClutch(outer_diameter=300, inner_diameter=225, cone_angle=0)
        with pytest.raises(ValueError):
            ConeClutch(outer_diameter=300, inner_diameter=225, cone_angle=60)
